...
def parser_input():
    # Parse arguments :
    parser = argparse.ArgumentParser(description="Check remaining simulation time.")
    ...
    for f in args.f:
        ...
        if len(files) > 0:
            ...
            if running:
                ...
                print(
                    f"{f:{max_str_size}} {sim_time:7.3f} us {run_perc:5.1f}% Complete in {int(expected_days):2d}:{int(expected_hours):02d}:{int(expected_minutes):02d} d:h:min ({ns_a_day:6.1f} ns/day)"
                )
